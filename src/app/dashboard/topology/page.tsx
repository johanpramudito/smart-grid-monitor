...
export default function TopologyPage() {
  ...
  const buildGraph = useCallback(
    (data: { nodes: ZoneNode[]; tieClosed?: boolean }) => {
      ...
      mainZones.forEach((zone, index) => {
        ...
        const isEnergized = !anyPreviousRelayOpen && status !== "OFFLINE" && deviceOnline;
        ...
        const relayState =
          !deviceOnline ? "UNKNOWN" :            // Device offline: state unknown
          status === "FAULT" ? "OPEN" :          // Fault: relay opens for protection
          status === "ISOLATED" ? "OPEN" :       // Isolated: relay open for isolation
          status === "OFFLINE" ? "UNKNOWN" :     // Offline: state unknown
      ...
      const tieIsEnergized = anyPreviousRelayOpen && tieIsClosed && tieDeviceOnline && tieStatus !== "OFFLINE";

      // Color for tie relay connections
      const tieConnectionColor = getConnectionColor(
        tieZone ?? { data: { status: "OFFLINE", deviceLastSeen: null } } as ZoneNode,
        tieIsEnergized
      );
      ...
      const lastRelayToTieColor = anyPreviousRelayOpen && tieIsClosed ? tieConnectionColor : "#64748b";
      const lastRelayToTieAnimate = anyPreviousRelayOpen && tieIsClosed && tieAnimate;
  ...
  const fetchTopology = useCallback(async (showLoading = false) => {
    if (showLoading) {
      setIsLoading(true);
    }
    try {
